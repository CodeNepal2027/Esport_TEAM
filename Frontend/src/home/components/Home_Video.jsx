...
const Home_Video = () => {
    ...
    // Hydrate YouTube metadata — newest first
    useEffect(() => {
        if (!rawVideos?.length) {
            setHydrating(false);
            return;
        ...
        (async () => {
            ...
            const hydrated = await Promise.all(
                // ⬇ reverse first so newest ends up on top after hydration
                [...rawVideos].reverse().map(async (video, index) => {
    ...
    const showMore = () => setVisibleCount(filteredVideos.length);
    const showLess = () => {
        setVisibleCount(getDefaultVisibleCount());
        document.getElementById('home-video-section')?.scrollIntoView({ behavior: 'smooth' });
    };
            ...
            <div className="container">
                        ...
                        visibleVideos.map((video) => (
                            ...
                            >
                                <div className="video-thumbnail-wrapper" onClick={() => openVideo(video)}>
                                    ...
                                    <span className="video-card-accent-bar"></span>
                                    ...
                                    <span className="video-duration-badge" style={{
                                        background: `linear-gradient(135deg, ${color_code_1}, ${color_code_2})`
                                    }}>
                ...
                {(hasMore || hasLess) && (
                    <div className="video-actions">
                        {hasMore && (
                            <button
                                className="video-view-btn video-view-more"
                                onClick={showMore}
                        ...
                        {hasLess && (
                            <button
                                className="video-view-btn video-view-less"
                                onClick={showLess}
                                style={{ '--btn-accent': color_code_1 }}
                            >
