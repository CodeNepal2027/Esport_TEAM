...
from Client.models import Hero, About, Sponsers, Gallery, Team, Events, Videos
...
# ============================================
# PUBLIC — section endpoints
# NOTE: every serializer call passes context={'request': request}
#       so image URLs come back as absolute (http://host/media/...).
# ============================================
...
def hero_view(request):
    ...
    qs = Hero.objects.filter(organization_slug=slug).order_by('order', 'id')
    return Response({
        'slides': HeroSerializer(qs, many=True, context={'request': request}).data
    })
...
def sponsors_view(request):
    ...
    qs = Sponsers.objects.filter(organization_slug=slug).order_by('order', 'id')
    return Response(
        SponsersSerializer(qs, many=True, context={'request': request}).data
    )
...
def gallery_view(request):
    ...
    qs = qs.order_by('order', 'id')
    return Response(
        GallerySerializer(qs, many=True, context={'request': request}).data
    )
...
def team_view(request):
    ...
    qs = Team.objects.filter(organization_slug=slug).order_by('order', 'id')
    return Response(
        TeamSerializer(qs, many=True, context={'request': request}).data
    )
...
def events_view(request):
    ...
    qs = qs.order_by('order', 'id')
    return Response(
        EventsSerializer(qs, many=True, context={'request': request}).data
    )
...
def videos_view(request):
    ...
    qs = qs.order_by('order', 'id')
    return Response(
        VideosSerializer(qs, many=True, context={'request': request}).data
    )


# ============================================
# ADMIN CRUD — ViewSets
# DRF automatically passes `request` into serializer context,
# so image URLs come back absolute here as well.
# ============================================
class _ScopedViewSet(viewsets.ModelViewSet):
    """
    Staff-only CRUD, scoped by ?slug= for filtering.
    Supports JSON + multipart (file uploads).
    """
    permission_classes = [IsAdminUser]
